...
/// (a phrase/word that reads the same backwards as it does forward. Assume all words are lowercase with no whitespace)
/// If any of the words is not a palindrome, return false
///
/// Example: arePalindromes([racecar, campusmotto:bottomsupmac, deified]) --> true
///          arePalindromes([ana, is, cool]) --> false
///
...
func arePalindromes(_ words: [String]) -> Bool {
    for word in words {
        ...
        var start = 0
        while start < end {
            let front = word.index(word.startIndex, offsetBy: start)
            let back = word.index(word.startIndex, offsetBy: end - 1)
...
func findStudent(withStudentID sid: Int?, studentIDNumbers: [Int: String] = getSIDsDictionary()) -> String? {
    ...
    // Note: studentIDNumbers[KEY] always returns an optional since there may not be an entry for that KEY, in which case studentIDNumbers[KEY] returns nil.
    
    // your code here
    if let newID = sid {
        if let nameFromDict = studentIDNumbers[newID] {
            return nameFromDict + " stole the cookie!"
        }
    }
    return nil
}
...
class Building: Equatable {
    ...
    static func == (lhs: Building, rhs: Building) -> Bool {
        return lhs.name == rhs.name && lhs.getCapacity() == rhs.getCapacity()
    }
...
var percentageStringClosure: (Double) -> String = {number in
